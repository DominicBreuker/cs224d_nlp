...
def normalizeRows(x):
    ...
    ### YOUR CODE HERE
    denom = np.sqrt(np.diag(x.dot(x.T)))
    x = x / denom[:, np.newaxis]
    ### END YOUR CODE
...
def softmaxCostAndGradient(predicted, target, outputVectors, dataset):
    ...
    ### YOUR CODE HERE
    # assume following dimensions
    # predicted: 1 x D 
    # target: scalar
    # outputVectors: W x D
    # dataset: ???
    scores = outputVectors.dot(predicted.T) # W x 1
    y_hat = softmax(scores) # W x 1
    cost = - np.log(y_hat[target]) # scalar

    tmp = y_hat # W x 1
    tmp[target] -= 1 # W x 1
    tmp = tmp[:, np.newaxis]
    gradPred = outputVectors.T.dot(tmp).T # 1 x D 

    grad = tmp.dot(predicted[np.newaxis, :]) # W x D
    ### END YOUR CODE
...
def skipgram(currentWord, C, contextWords, tokens, inputVectors, outputVectors, 
    ...
    for word in contextWords:
        ...
        j = tokens[word]
        cost_j, gradPred_j, grad_j = word2vecCostAndGradient(inputVectors[c], j, outputVectors, dataset)
        cost += cost_j
        gradIn[c,:] += gradPred_j[0,:]
        gradOut += grad_j
...
def cbow(currentWord, C, contextWords, tokens, inputVectors, outputVectors, 
    ...
    ### YOUR CODE HERE
    v_hat = np.zeros(inputVectors[0].shape)
    c = tokens[currentWord]
    for word in contextWords:
        j = tokens[word]
        v_hat += inputVectors[j]
    
    cost, gradPred, gradOut = word2vecCostAndGradient(v_hat, c, outputVectors, dataset)

    for word in contextWords:
        j = tokens[word]
        gradIn[j] += gradPred.flatten()
