...
class NERModel(LanguageModel):
  ...
  def add_embedding(self):
    ...
    with tf.device('/cpu:0'):
      ### YOUR CODE HERE
      L = tf.get_variable("Embedding", [len(self.wv), self.config.embed_size])
  ...
  def add_model(self, window):
    ...
    with tf.variable_scope("Layer"):
        W = tf.get_variable("W", [self.config.window_size * self.config.embed_size, self.config.hidden_size], initializer=xavier_weight_init())
        b1 = tf.get_variable("b1", [self.config.hidden_size])
        h = tf.tanh(tf.matmul(window, W) + b1)
        if self.config.l2:
            tf.add_to_collection("total_loss", 0.5 * self.config.l2 * tf.nn.l2_loss(W))
    with tf.variable_scope("Softmax"):
        U = tf.get_variable("U", [self.config.hidden_size, self.config.label_size], initializer=xavier_weight_init())
  ...
  def add_loss_op(self, y):
    ...
    """
    ### YOUR CODE HERE
    cross_entropies  = tf.nn.softmax_cross_entropy_with_logits(y, self.labels_placeholder)
