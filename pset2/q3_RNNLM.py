...
class RNNLM_Model(LanguageModel):
  ...
  def add_embedding(self):
    ...
    with tf.device('/cpu:0'):
      ...
      inputs = tf.nn.embedding_lookup(embedding, tf.transpose(self.input_placeholder)) # returns S x B x E
      inputs = [tf.squeeze(t, [0]) for t in tf.split(0, self.config.num_steps, inputs)] # returns list of lenght S, with tensors of shape B x E
      ### END YOUR CODE
      return inputs
  ...
  def add_projection(self, rnn_outputs):
    ...
    with tf.variable_scope("hidden"):
        ...
        outputs = [tf.matmul(rnn_output, U) + b_2 for rnn_output in rnn_outputs]
    ### END YOUR CODE
    return outputs
  ...
  def add_model(self, inputs):
    ...
    with tf.variable_scope("input_dropout"):
        inputs = [tf.nn.dropout(i, self.config.dropout) for i in inputs]

    with tf.variable_scope("RNN") as scope:
        self.initial_state = tf.zeros([self.config.batch_size, self.config.hidden_size])
        state = self.initial_state
        outputs = []
        for t, current_input in enumerate(inputs):
            if t > 0:
            ...
            state = tf.matmul(state, H) + tf.matmul(current_input, I) + b_1
            outputs.append(state)
    self.final_state = outputs[-1]

    with tf.variable_scope("output_dropout"):
        outputs = [tf.nn.dropout(o, self.config.dropout) for o in outputs]
